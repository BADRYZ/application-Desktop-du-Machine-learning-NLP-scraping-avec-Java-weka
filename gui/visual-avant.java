...
public class visual {
    public static void main(String[] args) {
        ...
        JPanel niveauEtudesPanel = createChartPanel("Niveau d'études Distribution", "Niveau d'études", "Count", "niveaudetude", "java");
        chartsPanel.add(niveauEtudesPanel);

        // Type de contrat Chart
        JPanel typeContratPanel = createChartPanel("Type de contrat Distribution", "Type de contrat", "Count", "typedecontrat", "java");
    ...
    private static JPanel createTablePanel() {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/avant_tr?useSSL=false", "root", "");
             Statement statement = connection.createStatement();
             ResultSet resultSet = statement.executeQuery("SELECT titre, datedepostule, region, secteurdactivite, typedecontrat, niveaudetude, experience, teletravail FROM java")) {
    ...
    private static DefaultPieDataset createTeletravailDataset() {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/avant_tr", "root", "");
             Statement statement = connection.createStatement();
             ResultSet resultSet = statement.executeQuery("SELECT teletravail, COUNT(*) FROM java GROUP BY teletravail")) {
    ...
    private static JPanel createExperienceChartPanel() {
        ...
        CategoryDataset dataset = createDataset("experience", "java");
